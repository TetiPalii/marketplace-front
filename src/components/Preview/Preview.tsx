...
export const Preview = () => {
    ...
    const URLs = useAppSelector(state => state.product.productURL);
  ...
  return (
      <section className="w-full px-4 py-5 h-full md:px-16">
          ...
          <Wrapper>
              ...
              {previewImg ? previewImg.map((url:string, index:number) => {
                  return <SwiperSlide key={index}>
                      <div className="relative w-full h-0 pb-[100%]">
                          <div className="absolute inset-0">
                              <Image src={url} width='250' height='250' alt="Foto preview" />
                              </div>
                          </div>
                      </SwiperSlide>
              }) : <p>
